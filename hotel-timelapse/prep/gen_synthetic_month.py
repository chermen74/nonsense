...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    p.add_argument("--no-sidecar", action="store_true",
                   help="Skip the .js sidecar the viewer uses when opened from file://")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args.no_sidecar:
        # A page opened from file:// may not fetch a sibling file, and
        # double-clicking public/index.html has to work. Same document, same
        # run, so the two cannot drift.
        sidecar = out.with_suffix(".js")
        sidecar.write_text(
            "/* Generated by prep/gen_synthetic_month.py -- do not edit.\n"
            f"   Offline copy of {out.name} for pages opened from file://. */\n"
            f"window.HOTEL_TIMELAPSE_DATA = {payload};\n",
            encoding="utf-8",
        )
        written.append(sidecar)

    s = doc["summary"]
    print(f"wrote {', '.join(str(w) for w in written)} -- {len(doc['days'])} days, "
